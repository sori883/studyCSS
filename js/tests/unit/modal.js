$(function () {
  ...
  window.Util = typeof simplicss !== 'undefined' ? simplicss.Util : Util
  ...
  QUnit.module('modal', {
    ...
    beforeEach: function () {
      ...
      $.fn.simplicssModal = $.fn.modal.noConflict()
    },
    afterEach: function () {
      $('.modal-backdrop, #modal-test').remove()
      $(document.body).removeClass('modal-open')
      $.fn.modal = $.fn.simplicssModal
      delete $.fn.simplicssModal
      $('#qunit-fixture').html('')
    }
  })
  ...
  QUnit.test('should expose defaults var for settings', function (assert) {
    assert.expect(1)
    assert.ok($.fn.simplicssModal.Constructor.Default, 'default object exposed')
  })
  ...
  QUnit.test('should toggle when toggle is called', function (assert) {
    ...
    $('<div id="modal-test"/>')
      .on('shown.sc.modal', function () {
        assert.ok($('#modal-test').is(':visible'), 'modal visible')
        assert.notEqual($('#modal-test').length, 0, 'modal inserted into dom')
        $(this).simplicssModal('toggle')
      })
      .on('hidden.sc.modal', function () {
        assert.ok(!$('#modal-test').is(':visible'), 'modal hidden')
        done()
      })
      .simplicssModal('toggle')
  })
  ...
  QUnit.test('should close reopened modal with [data-dismiss="modal"] click', function (assert) {
    ...
    $('<div id="modal-test"><div class="contents"><div id="close" data-dismiss="modal"/></div></div>')
      .one('shown.sc.modal', function () {
        $('#close').trigger('click')
      })
      .one('hidden.sc.modal', function () {
  ...
  QUnit.test('should not restore focus to toggling element if the associated show event gets prevented', function (assert) {
    ...
    $('<div id="modal-test"><div class="contents"><div id="close" data-dismiss="modal"/></div>')
      .one('show.sc.modal', function (e) {
        e.preventDefault()
        $otherBtn.trigger('focus')
        setTimeout($.proxy(function () {
          $(this).simplicssModal('show')
        }, this), 0)
      })
      .on('hidden.sc.modal', function () {
        setTimeout(function () {
  ...
  QUnit.test('should not follow link in area tag', function (assert) {
    ...
    $('#test')
      .on('click.sc.modal.data-api', function (event) {
  ...
  QUnit.test('should not try to open a modal which is already visible', function (assert) {
    ...
    $('<div id="modal-test"/>').on('shown.sc.modal', function () {
      count++
    }).on('hidden.sc.modal', function () {
  ...
  QUnit.test('should dispose modal', function (assert) {
    ...
    $modal.on('shown.sc.modal', function () {
      ...
      $(this).simplicssModal('dispose')

      var modalDataApiEvent = []
      $._data(document, 'events').click
        .forEach(function (e) {
          if (e.namespace === 'sc.data-api.modal') {
            modalDataApiEvent.push(e)
          }
        })

      assert.ok(typeof $(this).data('sc.modal') === 'undefined', 'modal data object was disposed')
  ...
  QUnit.test('should enforce focus', function (assert) {
    ...
    var modal = $modal.data('sc.modal')
    var spy = sinon.spy(modal, '_enforceFocus')
    var spyDocOff = sinon.spy($(document), 'off')
    var spyDocOn = sinon.spy($(document), 'on')

    $modal.one('shown.sc.modal', function () {
      assert.ok(spy.called, '_enforceFocus called')
      assert.ok(spyDocOff.withArgs('focusin.sc.modal'))
      assert.ok(spyDocOn.withArgs('focusin.sc.modal'))
